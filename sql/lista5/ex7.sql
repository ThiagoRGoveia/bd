...
INSERT INTO Disciplina VALUES('SM300', 'Calculo', 6, 30, 'Introdu��o ao C�lculo');
INSERT INTO Disciplina VALUES('SM400', 'Algebra', 4, 30, 'Introdu��o � Algebra');
...
-- ex 7
--antes
CREATE OR REPLACE FUNCTION matriculas_existentes_antes() RETURNS trigger AS $matriculas_existentes_antes$
BEGIN
DECLARE
numero_matriculas INTEGER;
SELECT COUNT(*) INTO numero_matriculas FROM Matricula;
RAISE NOTICE 'Numero de matriculas: %', numero_matriculas;

$matriculas_existentes_antes$ LANGUAGE plpgsql;

DROP TRIGGER matriculasExistentesAntes ON Matricula;

CREATE TRIGGER matriculasExistentesAntes
BEFORE INSERT OR UPDATE OR DELETE ON Matricula
FOR EACH ROW EXECUTE PROCEDURE matriculas_existentes_antes();

--depois
CREATE OR REPLACE FUNCTION matriculas_existentes() RETURNS trigger AS $matriculas_existentes$
BEGIN
DECLARE
numero_matriculas INTEGER;
SELECT COUNT(*) INTO numero_matriculas FROM Matricula;
...
$matriculas_existentes$ LANGUAGE plpgsql;

DROP TRIGGER matriculasExistentes ON Matricula;

CREATE TRIGGER matriculasExistentes
AFTER INSERT OR UPDATE OR DELETE ON Matricula
FOR EACH ROW EXECUTE PROCEDURE matriculas_existentes();
--fim do ex 7
