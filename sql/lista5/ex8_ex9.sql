...
-- ex 8
--Supondo que a tabela matricula existe
ALTER TABLE Matricula
ADD Aprovacao BOOLEAN DEFAULT FALSE;
--fim da alteracao da tabela matricula

CREATE OR REPLACE FUNCTION update_matricula() RETURNS trigger AS $update_matricula$
BEGIN
    PERFORM * FROM Matricula WHERE Nota >= 5.0;
    IF FOUND THEN 
        Aprovacao = TRUE;
    ELSE
        Aprovacao = FALSE;
    ...
    RETURN NULL;
END;
$update_matricula$ LANGUAGE plpgsql;

DROP TRIGGER alunosAprovados ON Matricula;

CREATE TRIGGER alunosAprovados 
AFTER INSERT OR UPDATE ON Matricula
FOR EACH ROW EXECUTE PROCEDURE update_matricula();
-- fim ex 8
