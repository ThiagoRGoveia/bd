-- O que eh pedido no ex 10 em si, está no final do arquivo
...
--ex 10
--Criando a funcao
CREATE OR REPLACE FUNCTION att_idade() RETURNS void AS $update_idade$
BEGIN
	-- ira atualizar a idade de todos os alunos da tabela
	UPDATE aluno SET idade = date_part('year',age(aluno.DataNasc));
END;
$update_idade$ LANGUAGE plpgsql;

-- funcao sendo executada
SELECT att_idade()
--fim do exercicio 10